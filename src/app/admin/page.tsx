    ...
    <div className="container py-10">
      ...
      {!isClerkConfigured() && (
        <div className="mt-4 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          ...
          <span>
            Clerk isn’t configured, so admin is <strong>open</strong> in this environment. Configure
            Clerk and <code>ADMIN_EMAILS</code> before deploying publicly.
          </span>
        </div>
      )}
            ...
            <tbody>
              {listings.map((l) => (
                <tr key={l.id} className="border-b last:border-0 align-top">
                  <td className="p-3">
                    <Link href={`/${l.slug}`} className="font-medium hover:underline">
                      {l.name}
                    </Link>
                    <div className="text-xs text-muted-foreground">{l.category?.name ?? '—'}</div>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <details>
                      ...
                      <form action={updateAffiliateAction} className="mt-2 flex flex-col gap-1">
                        ...
                        <input
                          name="affiliateUrl"
                          defaultValue={l.affiliateUrl ?? ''}
                          placeholder="https://affiliate-link…"
                          className="h-8 w-56 rounded-md border border-input bg-background px-2 text-xs shadow-sm"
                        />
                        <input
                          name="affiliateNetwork"
                          defaultValue={l.affiliateNetwork ?? ''}
                          placeholder="Network (Impact, Rewardful…)"
                          className="h-8 w-56 rounded-md border border-input bg-background px-2 text-xs shadow-sm"
                        />
                        <Button size="sm" variant="outline" className="h-8 w-20 text-xs">
                          Save
                        </Button>
                      </form>
                    </details>
